...
async function errorPlugin(fastify: FastifyInstance) {
  ...
  fastify.setNotFoundHandler((request, reply) => reply.code(404).error())
  ...
  function errorMethod(this: FastifyReply, options: ErrorOptions = {}) {
    ...
    return {
      error: {
        ...
        message: options.message || getDefaultErrorMessage(statusCode),
        info: options.info,
      },
    }
  }
...
function getDefaultErrorMessage(statusCode: number): string {
  switch (statusCode) {
    ...
    case 409:
      return 'Conflicting write request.'
    case 410:
